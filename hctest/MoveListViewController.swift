    ...
    func query(db:OpaquePointer, gameid:Int) {
        ...
        if sqlite3_prepare_v2(db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK
        {
            ...
            while sqlite3_step(queryStatement) == SQLITE_ROW
            {
                ...
                res.append(Result2(person: person, number: number, tense: tense, voice: voice, mood: mood, verbid: verbid, incorrectAns: incorrectString, elapsedTime: timeString, isCorrect: isCorrect))
                /*
                 print("Query Result:")
